...
function  ListTripsAdmPage() {
  ...
  return (
    <ContainerListTrips>
      <TripCard>
        {trips.map((trip) => {
          return <p key={trip.id}> {trip.name} - {trip.date} - {trip.planet} - {trip.durationInDays} dias {trip.description} <ButtonApply >Exibir Detalhes</ButtonApply></p>
        })}
